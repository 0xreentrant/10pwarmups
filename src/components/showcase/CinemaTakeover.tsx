...
interface CinemaTakeoverProps {
  deck: Deck
  videoSrc: string
  /** Review-page demo: full sequence on the scrubber, Train affordance, no quiz. */
  review?: boolean
}

export default function CinemaTakeover({ deck, videoSrc, review = false }: CinemaTakeoverProps) {
  ...
  return (
    <div>
      <CinemaStyles />
      <button type="button" className="ct-launcher" onClick={() => setOpen(true)}>
        <span className="ct-launcher-eyebrow">
          {review ? "review page demo · full sequence" : "Full-bleed takeover"}
        </span>
        <h2 className="ct-launcher-title">
          {review ? "Enter Cinema Review" : "Enter Cinema Mode"}
        </h2>
        <span className="ct-launcher-hint">
          {review
            ? "Replaces the move list with edge-to-edge tape. Tap sides to scrub, center to play/pause."
            : "Tap left/right to jump moves, tap center to play/pause"}
        </span>
      </button>

      {open && <CinemaOverlay deck={deck} videoSrc={videoSrc} review={review} onClose={() => setOpen(false)} />}
    </div>
  )
}
...
function CinemaOverlay({
  ...
  review = false,
  onClose,
}: CinemaTakeoverProps & { onClose: () => void }) {
    ...
    <div className="ct-overlay">
      ...
      {review && (
        <div className="ct-review-bar">
          ...
          <button type="button" className="ct-review-train" onClick={onClose}>Train</button>
        </div>
      )}

      <button type="button" className="ct-close" onClick={onClose} aria-label="Exit cinema mode">✕</button>
