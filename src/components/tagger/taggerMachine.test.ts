...
const PLAYERS2: Partner[][] = [["A"], ["A"]]

describe("taggerMachine", () => {
  it("SELECT sets selection and seeks; TIME does not clear selection", () => {
    const actor = start()
    actor.send({ type: "SET_DECK", deckId: "A1", moveCount: 3, moveNames: NAMES3, movePlayers: PLAYERS3 })
  ...
  it("holds seek until TIME is within epsilon or advances past", () => {
    const actor = start()
    actor.send({ type: "SET_DECK", deckId: "A1", moveCount: 2, moveNames: NAMES2, movePlayers: PLAYERS2 })
  ...
  it("RESET restores timestamps and moveNames from decks.ts defaults", () => {
    ...
    actor.send({ type: "SET_MOVE", index: 0, name: "Renamed", players: ["A"] })
    actor.send({
      type: "RESET",
    ...
    expect(ctx.movePlayers).toEqual([["A"], ["B"]])
  ...
  it("ADD_MOVE appends a move and selects it", () => {
    ...
    expect(ctx.movePlayers).toEqual([["A"], ["A"], ["A"]])
  ...
  it("DELETE_MOVE is a no-op when only one move remains", () => {
    const actor = start()
    actor.send({ type: "SET_DECK", deckId: "A1", moveCount: 1, moveNames: ["Only"], movePlayers: [["A"]] })
  ...
  it("REORDER_MOVE reorders names, players, timestamps, and selection", () => {
    const actor = start()
    actor.send({ type: "SET_DECK", deckId: "A1", moveCount: 3, moveNames: NAMES3, movePlayers: PLAYERS3 })
    actor.send({ type: "SEED", duration: 30, timestamps: [0, 10, 20] })
    actor.send({ type: "SELECT", index: 1 })
